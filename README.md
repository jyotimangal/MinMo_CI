...
## Folder Structure
Data, raw and processed, is stored on the netapp01data server (mbig/ directory, mounted as W:\ on Windows) within the KCL BMEIS network.
...
- `inventory_dicoms.py` saves a csv file with key metadata fields from all dicom files within the root directory W:\MinMo_CI\raw\ on `derivatives\`
